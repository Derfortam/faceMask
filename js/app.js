(() => {
    ...
    function initSliders(name) {
        if (document.querySelector(name)) new swiper_core_Swiper(".swiper", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
    ...
    function documentAction(e) {
        ...
        if (target.closest(".header__burger")) document.body.classList.toggle("menu-open");
    }
    window.addEventListener("scroll", function() {
        const myBlock = document.querySelector(".header");
        if (window.scrollY >= 100) myBlock.classList.add("active"); else myBlock.classList.remove("active");
    });
    let options = {
        root: null,
    ...
    let observerTwo = new IntersectionObserver(callback, otherOptions);
    let info = document.querySelector(".info");
    observerTwo.observe(info);
    const block1 = document.querySelector(".block1");
    observer.observe(block1);
    const block2 = document.querySelector(".block1");
    observer.observe(block2);
    const block3 = document.querySelector(".block1");
    observer.observe(block3);
    const someElements = document.querySelectorAll("[class*='--anim']");
